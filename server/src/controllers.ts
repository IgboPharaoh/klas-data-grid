...
export function updateOrder(orderId: number, update: Partial<DataObject>) {
  let index: number = 0;
  const order = data.find((item, i) => {
    index = i;
    return item.orderID === orderId;
  });
  if (order && Object.keys(update).length > 0) {
    Object.entries(order).forEach((keyValue) => {
